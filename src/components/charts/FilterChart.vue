...
export default class FilterChart extends Mixins(BaseMixin, ThemeMixin) {
    declare $refs: {
        filterchart: any
    }
    ...
    get yAxis() {
        return [
            {
                name: this.$t('Panels.FilterPanel.IndexesInChart'),
                type: 'value',
                min: 0,
                max: (value: any) => {
    ...
    get chart(): ECharts | null {
        return this.$refs.filterchart?.chart ?? null
    }
    ...
    get series() {
        const series = this.$store.state.printer.tempHistory.series.filter((series: { name: string | string[] }) =>
            series.name.includes('VOC')
        )
        return series ?? {}
    }
    ...
    get filterchartHeight() {
        return this.$store.state.gui.uiSettings.tempchartHeight ?? 250
    }

    get filterchartStyle() {
        return {
            height: this.filterchartHeight + 'px',
        }
    }
